...
function getUserByUsername(uname: any) {
  return Object.values(users).find(u => u.uname == uname);
}
...
function decorateComment(comment: any) {
  comment = {
    ...comment,
...
function addVote(userId, postId, value) {
  votes.push({
    "user_id": userId,
    "post_id": postId,
    "value": value
...
function removeVote(userId, postId) {
  const index = votes.findIndex(v => v.post_id == postId && v.user_id == userId);
...
function getVote(userId: number, postId: number) {
  return votes.find((v) => v.user_id == userId && v.post_id == postId);
}

function registerUser(uname: string, password: string) {
